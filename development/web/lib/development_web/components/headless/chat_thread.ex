defmodule DevelopmentWeb.Components.Headless.ChatThread do
  ...
  The `empty` part shows only while `messages` has no children (a Tailwind `group-has-*`
  variant, i.e. `:has()`), so it needs no assign and works with streams.
  ...
  **Documentation:** https://mishka.tools/chelekom/docs/headless/chat_thread
  ...
  def chat_thread(assigns) do
    ...
    <div
      ...
      data-on-top={@on_top}
      class={["chelekom-chat-thread group/thread relative flex min-h-0 flex-col", @class]}
      {@rest}
    >
      <div
        data-part="viewport"
        ...
        tabindex="0"
        class={[
          "chelekom-chat-thread__viewport min-h-0 flex-auto overflow-y-auto overscroll-contain",
          @viewport_class
        ]}
      >
        <div
          ...
          data-part="empty"
          class={[
            "chelekom-chat-thread__empty group-has-[[data-part=messages]>*]/thread:hidden",
            @empty_class
          ]}
        >
          {render_slot(@empty)}
